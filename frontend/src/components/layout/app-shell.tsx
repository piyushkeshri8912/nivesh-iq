...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`flex h-screen w-screen overflow-hidden ${pathname === "/" ? "bg-transparent" : "bg-zinc-950"} font-sans animate-fadeIn flex-col`}>
      {/* Top Navbar on Desktop / Bottom Tabbar on Mobile */}
      <Navbar />

      {/* Main Content Area */}
      <div className="flex-1 flex flex-row overflow-hidden relative">
        <main className={`flex-1 ${pathname === "/ask" ? "overflow-hidden" : "overflow-y-auto"} ${pathname === "/" ? "bg-transparent" : "bg-zinc-950"} pt-14 md:pt-16 pb-16 md:pb-0 transition-all duration-300`}>
          <div className={pathname === "/ask"
            ? "max-w-none w-full h-full p-0"
            : "max-w-7xl mx-auto p-6 sm:p-8 pb-24 md:pb-8"
          }>
            {children}
          </div>
        </main>
      ...
      {/* Floating assistant action sparkles button (hidden on /ask itself, when sidebar is open, and on mobile screens) */}
      {pathname !== "/ask" && !isChatOpen && (
        <button
